...
int main(int argc, char *argv[])
{
	...
	// 네트워크 바이트 순서로 사람이 읽을 수 있는 IP로 출력
	// 255까지의 숫자는 한바이트에 담겨서 순서만 바뀌고 값이 요상해지진 않음
	printf("IPv4 주소(네트워크 바이트 순서) = %d.%d.%d.%d\n",
		(ipv4num.s_addr & 0xFF000000) >> 24,
		(ipv4num.s_addr & 0x00FF0000) >> 16,
		(ipv4num.s_addr & 0x0000FF00) >> 8,
		(ipv4num.s_addr & 0x000000FF));
	printf("\n");
